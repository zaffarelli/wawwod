...
from collector.templatetags.wod_filters import as_tribe_plural, as_rank, as_breed, as_auspice
from collector.utils.wod_reference import (
    find_stat_property,
    STATS_NAMES,
    GM_SHORTCUTS,
    bloodpool,
...
class Creature(models.Model):
    ...
    reparts = models.CharField(max_length=32, blank=True, default="7_5_3__13_9_5")
    ...
    def fix_ghoul(self):
        ...
        self.display_gauge = int(int(self.trueage) / 50)
    ...
    def fix_kinfolk(self):
        ...
        TRIBES = [tribe for tribe in ALL_TRIBES if tribe not in DISCARDED_TRIBES]
    ...
    def fix(self):
        ...
        if "changeling" == self.creature: # Fae
        ...
        if "kindred" == self.creature: # Vampire
            ...
            self.fix_kindred()
        elif "ghoul" == self.creature:  # Ghoul
            self.freebies -= from_stats(self.creature, "traits") * 10
            self.freebies -= from_stats(self.creature, "virtues") * 2
            self.fix_ghoul()
        elif "garou" == self.creature: # Werewolf
            self.freebies -= from_stats(self.creature, "traits") * 7
        ...
        elif "kinfolk" == self.creature: # Kinfolk
            self.freebies -= from_stats(self.creature, "willpower")
            self.fix_kinfolk()
        elif "fomori" == self.creature: # Fomori
            self.freebies -= from_stats(self.creature, "willpower")
            self.fix_fomori()
        # Mage
    ...
    def roster_talents(self):
        ...
        for topic in topics:
            for ability in STATS_NAMES[self.creature][topic]:
                ...
                if val > 0:
                    abilities_list.append(f"{ability.title()} <tt>{val}</tt>")
    ...
    def randomize_fields(
        ...
        for x in range(partitions_count):
            array = self.randomize_stat(
                stats_count=partition_length, min_value=min, pool=int(points[x])
            )
    ...
    def randomize_backgrounds(self):
        ...
        for background in backgrounds:
            ...
            if background in favored:
                v["weight"] += 2
                v["status"] += "favored"
            elif background in discouraged:
                v["weight"] -= 2
                v["status"] += "discouraged"
            elif background in restricted:
                v["weight"] = 0
                v["status"] += "restricted"
            elif background in mandatory:
                #s = background.split("/")
        ...
        for v in vector:
            i = STATS_NAMES[self.creature]["backgrounds"].index(v["background"])
            if i != -1:
                setattr(self,f"background{i}", v["value"])

        # total_backgrounds = len(backgrounds)
        # self.randomize_fields(
        #     field=["background"], pool=5, partition_length=total_backgrounds, min=0
        # )
    ...
    def randomize_archetypes(self):
        ...
        if self.creature in ["kinfolk", "garou"]:
            source = "WTA"
        archetypes = Archetype.objects.filter(source__contains=source)
        ARCHS = []
        for archetype in archetypes:
            ARCHS.append(archetype.name)
        if not self.nature:
            self.nature = random.choice(ARCHS)
        if not self.demeanor:
            self.demeanor = random.choice(ARCHS)
    ...
    def extract_raw(self, options=["shortcuts"]):
        ...
        if self.creature == "garou":
            ...
            lines.append(f"Rank: {self.garou_rank}){new_line}")
        elif self.creature == "kindred":
        ...
        if len(self.nature) > 0 and len(self.demeanor) > 0:
            lines.append(f"__Nature/Demeanor__: {self.nature}/{self.demeanor}{new_line}")
        if len(self.concept) > 0:
            lines.append(f"__Concept__: {self.concept}{new_line}")
        if len(self.groupspec) > 0:
            lines.append(f"__Pack__: {self.groupspec}{new_line}")

        if self.is_player:
            lines.append(
                f"Attributes: {self.total_physical}/{rep[0]}:{self.total_social}/{rep[1]}:{self.total_mental}/{rep[2]}{new_line}"
            )
        # lines.append("```\n")

        lines.append(
            f"STR {self.attribute0:2} CHA {self.attribute3:2} PER {self.attribute6:2}{new_line}"
        )
        lines.append(
            f"DEX {self.attribute1:2} MAN {self.attribute4:2} INT {self.attribute7:2}{new_line}"
        )
        lines.append(
            f"STA {self.attribute2:2} APP {self.attribute5:2} WIT {self.attribute8:2}{new_line}"
        )
        # lines.append("\n")
        lines.append(f"__Willpower__:    {self.willpower:2}  {squared(self.willpower, 10)}{new_line}")
        if self.creature == "garou":
            lines.append(f"__Rage__:         {self.rage:2}  {squared(self.rage, 10)}{new_line}")
            lines.append(f"__Gnosis__:       {self.gnosis:2}  {squared(self.gnosis, 10)}{new_line}")
        elif self.creature == "kindred":
            lines.append(f"__Humanity__:     {self.humanity:2}  {squared(self.humanity, 10)}{new_line}")
            lines.append(f"__Bloodpool__:    {self.bloodpool:2}  {squared(self.bloodpool, 20)}{new_line}")
            lines.append(f"__Conscience__:   {self.virtue0:2}  ")
            lines.append(f"__Self-Control__: {self.virtue1:2}  ")
            lines.append(f"__Courage__:      {self.virtue2:2}{new_line}")
        if self.is_player:
            lines.append(
                f"Abilities: {self.total_talents}/{rep[3]}:{self.total_skills}/{rep[4]}:{self.total_knowledges}/{rep[5]}{new_line}"
            )
        tlines = []
        slines = []
        klines = []
        ...
        lines.append(f"__Talents__: {', '.join(tlines)}.{new_line}")
        lines.append(f"__Skills__: {', '.join(slines)}.{new_line}")
        lines.append(f"__Knowledges__: {', '.join(klines)}.{new_line}")
        ...
        lines.append(f"__Backgrounds__: {', '.join(blines)}.{new_line}")
        if self.creature == "garou":
            glines = []
            for n in range(16):
        ...
        if self.creature in ["garou", "kindred"]:
            ...
            if len(rlines) > 0:
                lines.append(f"__Rituals__: {', '.join(rlines)}.{new_line}")
            ...
            if len(self.edges) > 0:
                ...
                for edge in edges:
                    ...
                    if len(creatures) == 1:
                        ...
                        cstr += f"{new_line}" + creature.extract_raw(options=["shortcuts"])
                if len(cstr) == 0:
                    lines.append("Edges: " + self.edges + "")
                else:
                    lines.append(f"{new_line}" + cstr)
            # else:
            #     lines.append("**Edges**: none")
        # lines.append("xxx\n")
        lines.append(f"{self.challenge}{new_line}")
        return "".join(lines)
    ...
    def calculate_freebies(self):
        ...
        if "changeling" == self.creature:
            ...
            hdot = self.value_of(f"{self.creature}:freebies_per_dot:humanity")
            # Virtues
            k = 0
            for i in range(3):
                self.freebies += getattr(self, f"virtue{i}") * vdot
    ...
    def retainers(self):
        list = []
        if self.creature == "kindred":
            ghouls = Creature.objects.filter(sire=self.rid, creature="ghoul").order_by(
                "-trueage", "position"
            )
    ...
    def reghoul(self):
        ...
        if self.creature == "kindred":
            ...
            if background_retainers > 0:
                print(
                    f"{self.name} is supposed to have {background_retainers} ghoul(s)."
                )
                ...
                if missed_count:
                    ...
                    for x in range(missed_count):
                        ...
                        if self.trueage > 50:
                            retainer.trueage = self.trueage - randrange(
                                15, self.trueage - 15
                            )
            ...
            if len(domitors) == 0:
                ...
                print(
                    f"ERROR: {self.name} domitor has multiple occurences... {self.sire}"
                )

        else:
            print(
                "self.reghoul(): This is neither kindred nor ghoul, nothing to do to check."
            )
    ...
    def background_notes(self):
        ...
        for b in backgrounds:
            ...
            if v > 0:
                txt_lines = []
                entries = Background.objects.filter(
                    name=b.title(), level__lte=v
                ).order_by("level")
                i = 0
                if len(entries) > 0:
                ...
                x = "\n".join(txt_lines)
                fmt_list.append(
                    {"idx": idx, "item": f"{b.title()} [{v}] ", "notes": f"{x}"}
                )
                idx += 1
    ...
    def traits_notes(self):
        ...
        if self.creature in ["kindred", "ghoul"]:
            ...
            all = Discipline.objects.filter(code__in=extended_traits).order_by(
                "name", "level"
            )
    ...
    def nature_notes(self):
        ...
        if len(archetypes) == 1:
            ...
            fmt_list.append(
                {
                    "idx": 0,
                    "item": f"{archetype.name.upper()} [nature: what you are]",
        ...
        if len(archetypes) == 1:
            ...
            fmt_list.append(
                {
                    "idx": 1,
                    "item": f"{archetype.name.upper()} [demeanor: how you behave]",
    ...
    def randomize_traits(self):
        ...
        if "kindred" == self.creature:
            ...
            # print("Garou Rank", self.garou_rank)
            from collector.models.gifts import Gift
            for r in range(self.garou_rank):
                level = r + 1
                agifts = Gift.fetch(level=level, auspice=self.auspice)
                bgifts = Gift.fetch(level=level, breed=self.breed)
    ...
    def ritualistic_discipline(self):
        """
        :returns: the discipline and the path
        """
        result = ""
        if self.creature == "kindred":
            from collector.models.disciplines import Discipline
...
def randomize_reps(modeladmin, request, queryset):
    for creature in queryset:
        creature.randomize_reparts()
        creature.need_fix = True
        creature.save()
    short_description = "Randomize Reparts"
...
# def randomize_gifts(modeladmin, request, queryset):
#     for creature in queryset:
#         creature.randomize_traits()
#     short_description = "Randomize Gifts"
