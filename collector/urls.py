...
from collector.views.base import (
    index,
    get_list,
    updown,
    userinputastext,
    ...
    calendar,
    weaver_code,
    display_sept_rosters,
    quaestor,
    adventure_sheet,
    display_chronicle_map,
    display_edge_map,
    add_kinfolk,
    add_mortal,
    bulk_add,
...
from collector.views.actions import (
    change_settings,
    extract_mechanics,
    extract_per_group,
    extract_raw,
...
urlpatterns = [
    ...
    re_path(
        r"^ajax/list/creatures/(?P<pid>\d+)/(?P<slug>\w+)/$", get_list, name="get_list"
    ),
    re_path(
        r"^ajax/switch/chronicle/(?P<slug>\w+)/$",
        change_chronicle,
        name="change_chronicle",
    ),
    re_path(
        r"^ajax/switch/season/(?P<slug>\w+)/$",
        change_chronicle,
        name="change_chronicle",
    ),
    re_path(
        r"^ajax/switch/adventure/(?P<slug>\w+)/$",
        change_adventure,
        name="change_adventure",
    ),
    re_path(
        r"^ajax/display/gaia_wheel/$", display_gaia_wheel, name="display_gaia_wheel"
    ),
    re_path(r"^ajax/display/dashboard/$", display_dashboard, name="display_dashboard"),
    re_path(
        r"^ajax/display/chronicle_map/$",
        display_chronicle_map,
        name="display_chronicle_map",
    ),
    re_path(
        r"^ajax/display/edges_map/(?P<slug>\w+)/$",
        display_edge_map,
        name="display_edge_map",
    ),
    re_path(
        r"^ajax/display/groups/(?P<slug>\w+)/$", display_groups, name="display_groups"
    ),
    re_path(
        r"^ajax/display/adventure_sheet/$", adventure_sheet, name="adventure_sheet"
    ),
    re_path(
        r"^ajax/display/kindred_lineage/(?P<slug>\w+)/$",
        display_lineage,
        name="display_lineage",
    ),
    re_path(
    ...
    re_path(
        r"^ajax/display/crossover_sheet/$",
        display_crossover_sheet,
        name="display_crossover_sheet",
    ),
    re_path(
        r"^ajax/display/crossover_sheet/(?P<slug>\w+)/$",
        display_crossover_sheet,
        name="display_crossover_sheet",
    ),
    re_path(
        r"^ajax/display/crossover_sheet/(?P<slug>\w+)/(?P<option>\w+)/$",
        display_crossover_sheet,
        name="display_crossover_sheet",
    ),
    re_path(
        r"^ajax/view/creature/(?P<slug>\w+)/$",
        CreatureDetailView.as_view(),
        name="view_creature",
    ),
    ...
    re_path(
        r"^ajax/editable/userinput/as_text/$", userinputastext, name="userinputastext"
    ),
    re_path(
        r"^ajax/collector_action/add_garou/(?P<slug>\w+)/$", add_garou, name="add_garou"
    ),
    re_path(
        r"^ajax/collector_action/add_kinfolk/(?P<slug>\w+)/$",
        add_kinfolk,
        name="add_kinfolk",
    ),
    re_path(
        r"^ajax/collector_action/add_kindred/(?P<slug>\w+)/$",
        add_kindred,
        name="add_kindred",
    ),
    re_path(
    ...
    re_path(
        r"^ajax/collector_action/renown_records/$",
        renown_records,
        name="renown_records",
    ),
    ...
    re_path(
        r"^ajax/character/svg2pdf/(?P<slug>[\w-]+)/$", svg_to_pdf, name="svg_to_pdf"
    ),
    re_path(
        r"^ajax/character/save2svg/(?P<slug>[\w-]+)/$", save_to_svg, name="save_to_svg"
    ),
