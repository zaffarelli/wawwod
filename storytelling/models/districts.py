...
class District(models.Model):
    ...
    camarilla_intelligence = models.PositiveIntegerField(
    ...
    def populate(self):
        ...
        if chronicle:
            camarilla = Creature.objects.filter(
                chronicle=chronicle.acronym,
                faction__in=["Camarilla", "Anarchs"],
                creature="kindred",
                hidden=False,
                district=self.code,
            ).order_by("-freebies")
            independents = Creature.objects.filter(
                chronicle=chronicle.acronym,
            ...
            sabbat = Creature.objects.filter(
                chronicle=chronicle.acronym,
                faction="Sabbat",
                creature="kindred",
                hidden=False,
            ...
            for k in sabbat:
                self.population_details += (
                    f'<li><span class="sabbat">{k.name}</span> ({k.freebies})</li>'
                )
                self.population += 1
                inde_pop += k.freebies
            if inde_pop == 0:
    ...
    def buildPalette(self, size=16):
        ...
        for x in range(size):
            ...
            palette.append(
                f"#{round(color[0] * 255):02x}{round(color[1] * 255):02x}{round(color[2] * 255):02x}"
            )
        return palette
