...
class Story(models.Model):
    ...
    adventure = models.CharField(default="",blank=True, max_length=128)
    description = models.TextField(max_length=1024, blank=True, default="")
    dday = models.DateTimeField(default=timezone.now, blank=True, null=True)
    ...
    def all_timelines(self):
        ...
        for s in self.scene_set.all().order_by("timeline"):
            if s.timeline != timeline:
                ...
                cnt += 1
                timelines.append(
                    {"id": cnt, "label": s.timeline, "name": f"timeline{cnt}"}
                )
        return timelines
            ...
            list.append(
                {
                    "name": s.name,
                    "id": s.id,
                    "time": s.time_offset_hours,
                    "place": s.place.id,
                    "timeline": s.timeline,
                    "external_links": s.external_links,
                    "place_order": s.place_order,
                    "is_event": s.is_event,
                    "is_downtime": s.is_downtime,
                    "story_time": s.story_time,
                    "hours_duration": s.hours_duration,
                    "description": markdown.markdown(s.description),
                }
            )
        return list
            ...
            list.append(
                {
                    "category": s.category,
                    "id": s.id,
                    "scene_from": s.scene_from.id,
                    "scene_to": s.scene_to.id,
                    "description": s.description,
                    "valid": s.valid,
                    "order_in": s.order_in,
                    "order_out": s.order_out,
                }
            )
        return list
    ...
    def all_cast(self):
        ...
        lst = []
        all_scenes = Scene.objects.filter(story=self).order_by(
            "timeline", "time_offset_hours"
        )
    ...
    def calendar(self):
        ...
        for scene in all_scenes:
            ...
            if current_day == 0:
                ...
                scene_list.append(
                    f"<tr><th colspan=4>{calendar.day_name[scene.story_time.weekday()]} {scene.story_time.date()}</th></tr>"
                )
            if current_day == scene.time_offset_custom.split(" ")[0]:
                scene_list.append(
                    f"<tr><td width='3cm'>{scene.name}<br/>{scene.all_as_tags}</td><td width='3cm'>{scene.timeline}</td><td width='2cm'>{scene.place.acronym}</td><td width='1cm'>{scene.time_offset_hours % 24}h00</td></tr>"
                )
            else:
                current_day = scene.time_offset_custom.split(" ")[0]
                scene_list.append(
        ...
        txt = (
            "<center><table width='8cm' class='calendar'>"
            + "".join(scene_list)
            + "</table></center>"
        )
        return txt
